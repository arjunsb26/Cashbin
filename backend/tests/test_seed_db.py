...
def test_an_unfilled_asset_row_is_skipped_and_named(settings: Settings) -> None:
    init_db(settings)
    with session_scope() as session:
        summary = seed_all(session)
    assets = next(r for r in summary.results if r.table == "asset")
    assert assets.inserted == 0
    assert assets.skipped == 12
    assert len(assets.skipped_rows) == 12
    assert "BB-0002" in assets.skipped_rows[1]
    assert "cost_cents" in assets.skipped_rows[1]
    assert "in_service_date" in assets.skipped_rows[1]
    assert "tax_method" in assets.skipped_rows[1]


def test_placeholders_store_the_row_with_an_obvious_stand_in(settings: Settings) -> None:
    init_db(settings)
    with session_scope() as session:
        summary = seed_all(session, allow_placeholders=True)
    assets = next(r for r in summary.results if r.table == "asset")
